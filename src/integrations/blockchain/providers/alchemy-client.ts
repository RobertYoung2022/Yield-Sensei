/**
 * Alchemy Blockchain Client
 * Provides integration with Alchemy's blockchain infrastructure
 */
...
const logger = Logger.getLogger('alchemy-client');
...
interface AlchemyRPCResponse<T = any> {
  jsonrpc: string;
  id: number;
  result?: T;
  error?: {
...
export class AlchemyClient implements BlockchainClient {
  public readonly provider = 'alchemy' as const;
  ...
  private getBaseUrl(): string {
    const networkUrls: Record<string, string> = {
      ethereum: `https://eth-mainnet.g.alchemy.com/v2/${this.config.apiKey}`,
      polygon: `https://polygon-mainnet.g.alchemy.com/v2/${this.config.apiKey}`,
      arbitrum: `https://arb-mainnet.g.alchemy.com/v2/${this.config.apiKey}`,
      optimism: `https://opt-mainnet.g.alchemy.com/v2/${this.config.apiKey}`,
    };
  ...
  private async makeRPCCall<T>(method: string, params: any[] = []): Promise<BlockchainResponse<T>> {
    try {
      const request: AlchemyRPCRequest = {
        jsonrpc: '2.0',
        method,
        params,
        id: ++this.requestId
      ...
      const response = await this.client.post<AlchemyRPCResponse<T>>('', request);
  ...
  async getTokenBalance(address: string, tokenAddress: string): Promise<BlockchainResponse<TokenBalance>> {
    // Use Alchemy's enhanced API for token balance
    try {
      const response = await this.client.get(`/v2/${this.config.apiKey}/getTokenBalances`, {
        params: {
          owner: address,
      ...
      if (response.data && response.data.tokenBalances && response.data.tokenBalances.length > 0) {
        const tokenBalance = response.data.tokenBalances[0];
        
        // Get token metadata
        const metadataResponse = await this.client.get(`/v2/${this.config.apiKey}/getTokenMetadata`, {
          params: {
      ...
      // Fallback to standard ERC-20 call
  ...
  async getTokenPrice(tokenAddress: string): Promise<BlockchainResponse<PriceData>> {
    // Alchemy doesn't provide direct price data, would need external oracle
    return {
      success: false,
      error: 'Token price fetching not implemented in Alchemy client',
      metadata: {
        provider: this.provider,
        network: this.network,
        timestamp: Date.now()
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      logger.warn('Alchemy health check failed', { error });
      return false;
    }
  }
  ...
  private setupInterceptors(): void {
    ...
    this.client.interceptors.request.use(
      ...
      (error) => {
        logger.error('Alchemy request error', { error });
    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        logger.error('Alchemy response error', { 
