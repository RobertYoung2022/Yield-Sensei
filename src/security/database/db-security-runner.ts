#!/usr/bin/env node

/**
 * Database Security Runner
 * CLI tool for running database security validation
 */

import { DatabaseSecurityValidator } from './db-security-validator';
import { writeFileSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';
import { program } from 'commander';

class DbSecurityRunner {
  private validator: DatabaseSecurityValidator;

  constructor(projectRoot?: string) {
    this.validator = new DatabaseSecurityValidator(projectRoot);
  }

  async runValidation(): Promise<void> {
    console.log('🗄️  YieldSensei Database Security Validator\n');

    try {
      const summary = await this.validator.validateAll();
      
      console.log('\n📊 Database Security Validation Results');
      console.log('========================================\n');
      
      console.log(`✅ Tests Passed:       ${summary.passed}/${summary.totalTests}`);
      console.log(`❌ Tests Failed:       ${summary.failed}/${summary.totalTests}`);
      console.log(`📈 Success Rate:       ${summary.successRate.toFixed(2)}%\n`);
      
      if (summary.criticalIssues > 0 || summary.highIssues > 0) {
        console.log('🚨 Issues by Severity:');
        if (summary.criticalIssues > 0) {
          console.log(`   🔴 Critical:        ${summary.criticalIssues}`);
        }
        if (summary.highIssues > 0) {
          console.log(`   🟠 High:            ${summary.highIssues}`);
        }
        if (summary.mediumIssues > 0) {
          console.log(`   🟡 Medium:          ${summary.mediumIssues}`);
        }
        console.log('');
      }
      
      if (summary.recommendations.length > 0) {
        console.log('💡 Top Recommendations:');
        summary.recommendations.slice(0, 5).forEach(rec => {
          console.log(`   - ${rec}`);
        });
        console.log('');
      }
      
      // Save detailed report
      const reportDir = join(process.cwd(), 'reports');
      if (!existsSync(reportDir)) {
        mkdirSync(reportDir, { recursive: true });
      }
      
      const reportPath = join(reportDir, 'database-security-report.md');
      const report = this.validator.generateReport();
      writeFileSync(reportPath, report);
      console.log(`📄 Detailed report saved to: ${reportPath}\n`);
      
      // Show category breakdown
      this.showCategoryBreakdown();
      
      // Exit with appropriate code
      const exitCode = summary.criticalIssues > 0 ? 2 : summary.highIssues > 0 ? 1 : 0;
      
      if (exitCode === 0) {
        console.log('✅ Database security validation completed successfully');
      } else if (exitCode === 1) {
        console.log('🟠 Database security validation completed with high priority issues');
      } else {
        console.log('🔴 Database security validation completed with CRITICAL issues');
      }
      
      process.exit(exitCode);
      
    } catch (error) {
      console.error('❌ Database security validation failed:', error);
      process.exit(1);
    }
  }

  async runQuickScan(): Promise<void> {
    console.log('🗄️  Quick Database Security Scan\n');

    try {
      // Run only critical tests for quick scan
      const validator = new DatabaseSecurityValidator();
      
      // Manually run specific tests
      await validator['testConnectionSecurity']();
      await validator['testSqlInjectionPrevention']();
      await validator['testAccessControls']();

      const results = validator.getResults();
      const criticalResults = results.filter(r => !r.passed && r.severity === 'critical');
      const highResults = results.filter(r => !r.passed && r.severity === 'high');

      console.log('🔍 Quick Scan Results:');
      console.log(`   🔴 Critical Issues: ${criticalResults.length}`);
      console.log(`   🟠 High Issues:     ${highResults.length}`);
      console.log(`   📊 Tests Run:       ${results.length}\n`);

      if (criticalResults.length > 0) {
        console.log('🚨 Critical Issues Found:');
        criticalResults.forEach(result => {
          console.log(`   - ${result.testName}: ${result.recommendations[0] || 'Needs attention'}`);
        });
        console.log('');
      }

      const exitCode = criticalResults.length > 0 ? 2 : highResults.length > 0 ? 1 : 0;
      process.exit(exitCode);

    } catch (error) {
      console.error('❌ Quick scan failed:', error);
      process.exit(1);
    }
  }

  async generateReport(): Promise<void> {
    console.log('📄 Generating Database Security Report...\n');

    try {
      const summary = await this.validator.validateAll();
      const report = this.validator.generateReport();
      
      const reportDir = join(process.cwd(), 'reports');
      if (!existsSync(reportDir)) {
        mkdirSync(reportDir, { recursive: true });
      }
      
      const reportPath = join(reportDir, 'database-security-report.md');
      writeFileSync(reportPath, report);
      
      console.log(`✅ Report generated: ${reportPath}`);
      console.log(`📊 Summary: ${summary.passed}/${summary.totalTests} tests passed (${summary.successRate.toFixed(2)}%)`);
      
    } catch (error) {
      console.error('❌ Report generation failed:', error);
      process.exit(1);
    }
  }

  private showCategoryBreakdown(): void {
    const results = this.validator.getResults();
    const categories = ['connection', 'injection', 'access', 'encryption', 'monitoring'];
    
    console.log('📋 Results by Category:');
    categories.forEach(category => {
      const categoryResults = results.filter(r => r.category === category);
      const passed = categoryResults.filter(r => r.passed).length;
      const total = categoryResults.length;
      
      if (total > 0) {
        const icon = passed === total ? '✅' : passed > total / 2 ? '⚠️' : '❌';
        console.log(`   ${icon} ${category.charAt(0).toUpperCase() + category.slice(1)}: ${passed}/${total}`);
      }
    });
    console.log('');
  }
}

// CLI Setup
program
  .name('db-security-runner')
  .description('Database Security Validation Tool')
  .version('1.0.0');

program
  .command('validate')
  .description('Run full database security validation')
  .option('-p, --project <path>', 'Project root path')
  .action(async (options) => {
    const runner = new DbSecurityRunner(options.project);
    await runner.runValidation();
  });

program
  .command('quick')
  .description('Run quick database security scan')
  .option('-p, --project <path>', 'Project root path')
  .action(async (options) => {
    const runner = new DbSecurityRunner(options.project);
    await runner.runQuickScan();
  });

program
  .command('report')
  .description('Generate database security report')
  .option('-p, --project <path>', 'Project root path')
  .action(async (options) => {
    const runner = new DbSecurityRunner(options.project);
    await runner.generateReport();
  });

// Run if called directly
if (require.main === module) {
  // Default to validate command if no arguments
  if (process.argv.length === 2) {
    const runner = new DbSecurityRunner();
    runner.runValidation();
  } else {
    program.parse();
  }
}

export { DbSecurityRunner };