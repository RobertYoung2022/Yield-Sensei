...
import { ExecutionPathOptimizer, PathOptimizationConfig } from '../execution-path-optimizer';
import { GasOptimizer } from '../gas-optimizer';
import { BridgeFeeOptimizer } from '../bridge-fee-optimizer';
import { ExecutionTimeOptimizer } from '../execution-time-optimizer';
import { SlippageMinimizer } from '../slippage-minimizer';
import { 
  ExecutionPath, 
  ExecutionStep, 
...
describe('ExecutionPathOptimizer', () => {
  ...
  describe('initialization', () => {
    ...
    it('should throw error with invalid configuration', () => {
      ...
      expect(() => new ExecutionPathOptimizer(invalidConfig, mockChainConfigs, mockBridgeConfigs))
        .toThrow();
    });
  });
  ...
  describe('path optimization', () => {
    it('should optimize a valid arbitrage opportunity', async () => {
      ...
      expect(result.id).toContain('optimized');
    ...
    it('should handle complex multi-step paths', async () => {
      const complexOpportunity = {
        ...mockOpportunity,
        executionPaths: [{
          ...mockOpportunity.executionPaths[0],
          steps: [
      ...
      expect(result).toBeDefined();
      expect(result.steps.length).toBeGreaterThanOrEqual(complexOpportunity.executionPaths[0].steps.length);
    });
  });
  ...
  describe('error handling', () => {
    it('should handle invalid opportunity gracefully', async () => {
      ...
      await expect(optimizer.optimizePath(invalidOpportunity)).rejects.toThrow();
    });
    ...
    it('should handle optimization failures gracefully', async () => {
      ...
      await expect(optimizer.optimizePath(problematicOpportunity)).rejects.toThrow();
    });
    ...
    it('should validate input parameters', async () => {
      ...
      await expect(optimizer.optimizePath(opportunityWithInvalidSteps)).rejects.toThrow();
    });
  });
